...
pub fn generate_code_reporting(project: &Project, options: CodegenOptions) -> (String, CodegenReport) {
        ...
        match r.kind {
            ...
            RouteKind::Page | RouteKind::Html => {
                let (body, metadata, gen_params, revalidate, dynamic_params, tags) = if r.kind == RouteKind::Html {
                    ...
                    if info.consts.contains_key("RENDERING") {
                        // Read from the source by the build; referenced here so the
                        // compiler checks its type and doesn't report it as unused.
                        g.emit_once(
                            &format!("__rendering_{m}"),
                            format!("const _: __nr::Rendering = {m}::RENDERING;\n"),
                        );
                    }
    ...
    for (path, idx) in &g.modules {
        let _ = writeln!(header, "#[allow(non_snake_case)]\n#[path = {}]\nmod m{idx};", lit(&path.to_string_lossy()));
    }
    header.push('\n');
