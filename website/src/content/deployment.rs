...
pub fn content() -> Node {
    fragment![
        ...
        p![
            "The binary is built with fat link-time optimization, a single codegen unit and stripped symbols. A new project's server is around 2 MB. Large files in ",
            code!["public/"],
            ...
            table![
                ...
                tbody![
                    ...
                    tr![
                        ...
                        td!["readable source"],
                        td!["minified with local names mangled (~2.9 KB gzipped)"],
                    ],
                    tr![td!["error pages"], td!["full details"], td!["generic message and a digest"]],
                    tr![
                        td!["server binary"],
                        td!["debug info, reads files from disk"],
                        td!["one stripped file with everything embedded (fat LTO, single codegen unit)"],
                    ],
                ],
            ],
        ],
...
docker run -p 3000:3000 my-app",
        ...
        p![
            "The built-in server speaks HTTP/1.1 and HTTP/2 (cleartext, via prior knowledge or a proxy). Terminate ",
            strong!["TLS"],
            " and ",
            strong!["HTTP/3"],
    ...
    reverse_proxy 127.0.0.1:3000
        ...
        p!["Native TLS and HTTP/3 in the server are on the roadmap (", a![href("/docs/status"), "status.md"], ").",],
