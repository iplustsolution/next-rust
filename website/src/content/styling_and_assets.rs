...
article![class(styles.card), h2![class(styles.card_title), "Hello"]]"#,
        ...
        p![
            ...
            " sheet is also sent ",
            strong!["per page"],
            ": a document gets the rules for the classes it renders (and those in ",
            code!["[tailwind] keep_classes"],
            " and ",
            code!["[assets] css_safelist"],
            ", for classes that scripts add); rules without a class owner are always sent. See ",
            a![href("/docs/tailwind#per-page-css"), "Only the CSS a page uses"],
            ".",
        ],
        p![
        ...
        h3![id("unused-css"), a![class("anchor"), href("#unused-css"), "Unused CSS is left out"]],
        p![
            "Release builds (",
            code!["next-rust build"],
            ", ",
            code!["cargo build --release"],
            ") keep only the CSS your app uses. A 3,000-line stylesheet whose views use 45 classes ships the rules for those 45 classes. The build collects every name that appears in ",
            code!["app/"],
            ", ",
            code!["src/"],
            ", ",
            ...
            " and ",
            code!["css_module!"],
            " drop the rules whose classes or ids appear nowhere:",
        ],
        ul![
            li![
                "A rule stays if any selector in its list is used. Element selectors (",
                code!["body"],
                ", ",
                code!["h1"],
                ", ",
                code!["a:hover"],
                ") and attribute selectors always stay.",
            ],
            li![
                "Rules inside ",
                code!["@media"],
                ...
                " and ",
                code!["@container"],
                " are pruned the same way; a block left empty is removed. ",
                code!["@keyframes"],
                " no rule refers to any more are removed too.",
            ],
            li![
                "Names inside ",
                code![":is()"],
                ...
                " and ",
                code![":where()"],
                " never remove a rule. When in doubt, a rule is kept.",
            ],
            li!["Development builds keep everything, so a class you just typed works without restarting anything.",],
        ],
        p![
            "Tailwind CSS works the same way from the start: it only generates the classes you write (see ",
        ...
        div![
            ...
            p![
                ...
                code!["format!(\"theme-{name}\")"],
                ", a class read from a database) can't be found in the source. List them in ",
                code!["[assets] css_safelist"],
                ", or turn pruning off with ",
                code!["prune_css = false"],
                ". ",
                code!["NEXT_RUST_PRUNE_CSS=1"],
                " prunes in a development build too (to check the result), ",
                code!["NEXT_RUST_PRUNE_CSS=0"],
                " never prunes.",
            ],
        ],
        pre![code![
            class("language-toml"),
            "[assets]\nprune_css = true                          # default\ncss_safelist = [\"theme-dark\", \"theme-sepia\"]",
        ],],
...
img![src(asset!("images/logo.svg")), alt("Acme")]"#,
        ...
        p![
            ...
            code!["/_next-rust/assets/fonts/inter.<16-hex-hash>.woff2"],
